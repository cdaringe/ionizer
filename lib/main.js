...
var checkForInstalledHeaders = function(opts) {
    ...
    var canary = path.join(opts.headersDir, '.node-gyp', opts.electronVersion, 'common.gypi');
    return fs.statAsync(canary)
    .then(function(stat) {
        if (!stat) {
...
var installHeaders = function(opts) {
    if (!opts) {
        throw new ReferenceError('node header installation opts required');
    }
    if (typeof opts === 'string') {
        opts = {
            electronVersion: arguments[0],
            nodeDistUrl: arguments[1],
            headersDir: arguments[2],
            arch: arguments[3],
        };
    };
    opts.headersDir = opts.headersDir || getHeadersRootDirForVersion(opts.electronVersion);
    opts.nodeDistUrl = opts.nodeDistUrl || 'https://gh-contractor-zcbenz.s3.amazonaws.com/atom-shell/dist';

    return checkForInstalledHeaders(opts)
    .catch(function(err) {
        var cmd = 'node';
        var args = [
            ...
            '--dist-url=' + opts.nodeDistUrl
        ];

        return spawnWithHeadersDir(cmd, args, opts.headersDir);
    });
};
...
var shouldRebuild = function(electronPath) {
    ...
    .then(function assessRebuild() {
        result.shouldRebuild = process.versions.modules != result.electronModuleVersion;
        return result;
    });

};

var rebuild = function(cfg) {
    var opts = _.isString(cfg) ? { electronVersion: cfg } : _.assign({}, cfg);
    ...
    .then(assignElectronModuleVersion)
    .then(function() {
        return _.partial(pUtil.squashNode, opts.electronPath);
    })
    .then(rebuild);
};
