...
var spawnWithHeadersDir = function(cmd, args, headersDir, cwd) {
    ...
    winston.debug([
      ...
      'home:' + opts.env.HOME,
      'cwd:' + opts.cwd,
      'electron-as-node: ' + cp.spawnSync('which', ['node']).stdout.toString()
    ].join('\t\n'));
    return spawn({ cmd: cmd, args: args, opts: opts })
    .catch(function(err) {
        if (err.stdout) {
...
var rebuildQuick = function(opts) {
    ...
    return Promise.all([
        getInstalledModules({ modulesDir: opts.modulesDir, followSymLinks: opts.followSymLinks }),
