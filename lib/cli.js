...
.option('--verbose [level]', '[optional] turn ionizer into a chatty cathy')
.parse(process.argv);

if (program.verbose) {
    winston.level = program.verbose === true ? 'verbose' : program.verbose;
}
...
.then(function(result) {
    ...
    return installHeaders(result.electronVersion, null, null, program.arch);
})
.then(function() {
    var opts = {
        arch: program.arch,
