 ...
 * @param {object} opts
 * @param {string[]} opts.filenames list of file or folder names
 * @param {string} opts.modulesDir
 * @param {boolean} opts.followSymLinks
 * @return {Promise} resolves => ['names', 'of', 'folders']
 */
function filterFsFolders(opts) {
    var fnames = opts.filenames;
    var modulesDir = opts.modulesDir;
    var followSymLinks = opts.followSymLinks;
    var stats = fnames.map(function(file) {
        return fs.lstatAsync(path.join(modulesDir, file));
    });
    return Promise.all(stats)
    .then(function(stats) {
        return stats.map(function(stat, ndx)  {
            var isSymLink = stat.isSymbolicLink();
            if (stat.isDirectory() || (isSymLink && followSymLinks)) {
                if (isSymLink) { winston.warn('sym link detected. rebuilding, but may lead to unpredictable behavior'); }
                return {
                    package: fnames[ndx],
                    stat: stat
                };
 ...
 */
function getInstalledModules(opts) {
    var modulesDir = opts.modulesDir;
    var followSymLinks = opts.followSymLinks;
    ...
    .then(function(nm) {
        ...
        return filterFsFolders({ filenames: contents, modulesDir: modulesDir, followSymLinks: followSymLinks });
    });
}
