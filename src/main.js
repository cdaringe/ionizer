...
export async function shouldRebuild(pathToElectronExecutable, explicitNodeVersion=null) {
...
export async function rebuildNativeModulesQuick(opts) {
  ...
  if (!toBuild.length) {
      console.log(`ionizer: all builds up-to-date`);
  ...
  try {
    ...
    await toBuild.reduce((prev, bld, ndx)=> {
       ...
       return prev.then(last => {
          console.log(`ionizer: ${buildNdx}/${total}, ${bld.package}`);
    ...
    console.log('ionizer: updating build cache');
